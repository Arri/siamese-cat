...
I am running the code in an Anaconda environment using Python 3.9. I have Anaconda installed in an Ubuntu (wsl2) environment. To install all required packages in this environment, make the script `conda_install_packages.sh` executable (e.g. with `chmod +x conda_install_packages.sh`) and run it with
```bash
./conda_install_packages.sh
```
This will create an environment with the naming **cats**, activate that environment and install all required packages in that virtual environment.
